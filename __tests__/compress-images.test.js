...
const { compressImages } = require("../src/tree/compress-imags");
...
test("compressImages 2", () => {
  const tree = mkdir("my documents", [mkdir("presentations")]);

  const newTree = compressImages(tree);

  expect(newTree).toMatchObject({
    meta: {},
    children: [{ name: "presentations" }],
  });
});
...
test("compressImages 3 - deepClone", () => {
  const tree = mkdir(
    ...
    [
      mkfile("avatar.jpg", {
        size: 100,
        attributes: { hide: false, readOnly: true },
      }),
      mkdir("presentations"),
    ],
    { owner: "hexlet" }
